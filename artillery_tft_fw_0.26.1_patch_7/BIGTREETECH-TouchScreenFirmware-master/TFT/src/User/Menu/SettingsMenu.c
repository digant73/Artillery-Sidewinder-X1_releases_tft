...
#if !defined(HARDWARE_TEXT) || !defined(FIRMWARE_TEXT)
...
#else
  const char* hardware = "Board   : "STRINGIFY(HARDWARE_TEXT);
  const char* firmware = "Firmware: "STRINGIFY(FIRMWARE_TEXT)"." STRINGIFY(SOFTWARE_VERSION) " " __DATE__;
#endif
